-- A provider can be paid without becoming a staff account. Their commission is
-- an expense linked to the sale, preserving the real margin of the service.
...
begin
  ...
  if exists (select 1 from jsonb_to_recordset(p_items) as item(external_provider_name text, commission_cents integer) where nullif(trim(coalesce(external_provider_name, '')), '') is not null and coalesce(commission_cents, 0) <= 0) then raise exception 'Indica una comisión válida para el prestador externo'; end if;
  if exists (select 1 from jsonb_to_recordset(p_items) as item(external_provider_name text, specialist_id uuid) where nullif(trim(coalesce(external_provider_name, '')), '') is not null and specialist_id is not null) then raise exception 'Una comisión sólo puede pertenecer al equipo o a un prestador externo'; end if;
  ...
  insert into public.specialist_earnings(sale_id, specialist_id, service_id, amount_cents, paid_at, paid_by) select v_sale_id, item.specialist_id, null, item.commission_cents, now(), auth.uid() from jsonb_to_recordset(p_items) as item(service_id uuid, product_id uuid, specialist_id uuid, commission_cents integer) join public.profiles specialist on specialist.id=item.specialist_id and specialist.active and specialist.role='specialist' where item.service_id is null and item.product_id is null and coalesce(item.commission_cents, 0) > 0;
  if exists (select 1 from jsonb_to_recordset(p_items) as item(service_id uuid, product_id uuid, specialist_id uuid, commission_cents integer, external_provider_name text) where item.service_id is null and item.product_id is null and coalesce(item.commission_cents,0) > 0 and item.specialist_id is null and nullif(trim(coalesce(item.external_provider_name, '')), '') is null) then raise exception 'Selecciona al equipo o indica el prestador externo'; end if;
  insert into public.expenses(category, description, amount_cents, expense_date, payment_method, created_by, sale_id, external_provider_name) select 'Comisión externa', concat('Comisión de ', trim(item.external_provider_name), ' por ', trim(item.description)), item.commission_cents, current_date, item.external_payment_method, auth.uid(), v_sale_id, trim(item.external_provider_name) from jsonb_to_recordset(p_items) as item(description text, commission_cents integer, external_provider_name text, external_payment_method public.payment_method) where nullif(trim(coalesce(item.external_provider_name, '')), '') is not null and item.commission_cents > 0;
