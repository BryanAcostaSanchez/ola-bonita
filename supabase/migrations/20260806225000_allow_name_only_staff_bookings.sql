-- Las citas creadas desde la agenda interna pueden registrar una clienta con
-- solo su nombre; teléfono y datos adicionales siguen siendo opcionales.
...
begin
  ...
  if nullif(trim(p_full_name), '') is null then raise exception 'El nombre de la clienta es obligatorio'; end if;
  ...
  values (trim(p_full_name), nullif(trim(coalesce(p_phone, '')), ''), nullif(trim(coalesce(p_notes, '')), ''))
