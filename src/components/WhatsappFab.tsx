"use client";

import { useEffect, useState } from "react";
import type { Locale } from "@/lib/i18n/locale";
import { dictionary } from "@/lib/i18n/dictionary";
import { getWhatsappLink } from "@/lib/social";

export function WhatsappFab({ locale }: { locale: Locale }) {
  const t = dictionary[locale];
  const [showPrompt, setShowPrompt] = useState(true);

  useEffect(() => {
    const hidePrompt = () => setShowPrompt(false);
    const timer = window.setTimeout(hidePrompt, 6000);

    window.addEventListener("scroll", hidePrompt, { passive: true, once: true });
    return () => {
      window.clearTimeout(timer);
      window.removeEventListener("scroll", hidePrompt);
    };
  }, []);

  return (
    <div className="whatsapp-widget">
      <span
        className={`whatsapp-reserve-message${showPrompt ? " is-visible" : ""}`}
        aria-hidden={!showPrompt}
      >
        {t.announcement.text}
      </span>
      <a
        className="whatsapp-fab"
        href={getWhatsappLink(locale)}
        target="_blank"
        rel="noopener noreferrer"
        aria-label={t.whatsappFab.tooltip}
      >
        <span className="whatsapp-fab-ring" aria-hidden="true"></span>
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.868-2.03-.967-.273-.099-.472-.148-.67.15-.198.297-.767.966-.94 1.164-.173.198-.347.223-.644.075-.297-.149-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.372-.025-.521-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12.004 2C6.486 2 2 6.486 2 12.004c0 1.85.497 3.647 1.442 5.226L2 22l4.916-1.412a9.98 9.98 0 0 0 5.088 1.392c5.518 0 10.004-4.486 10.004-10.004S17.522 2 12.004 2Zm0 18.187a8.16 8.16 0 0 1-4.163-1.14l-.298-.177-2.917.838.85-2.844-.194-.293a8.174 8.174 0 0 1-1.278-4.567c0-4.516 3.674-8.19 8.19-8.19 4.517 0 8.19 3.674 8.19 8.19 0 4.517-3.673 8.183-8.19 8.183Z"/></svg>
        <span className="whatsapp-fab-badge" aria-hidden="true">1</span>
        <span className="whatsapp-fab-tooltip">{t.whatsappFab.tooltip}</span>
      </a>
    </div>
  );
}
