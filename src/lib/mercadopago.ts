...
type CheckoutBooking = { id: string; public_code: string; online_payment_cents: number; online_payment_kind: "deposit" | "full"; customer: { full_name: string; email: string | null; phone: string | null }; service: { name: string } };
...
export async function createCheckoutPreference(booking: CheckoutBooking) {
  ...
  const response = await fetch("https://api.mercadopago.com/checkout/preferences", {
    ...
    body: JSON.stringify({
      items: [{ id: booking.id, title: `${booking.online_payment_kind === "full" ? "Pago completo" : "Anticipo"} · ${booking.service.name}`, quantity: 1, currency_id: "MXN", unit_price: booking.online_payment_cents / 100 }],
...
export async function createCabinCheckout(reservation: { id: string; public_code: string; online_payment_cents: number; online_payment_kind: "deposit" | "full"; full_name: string; email: string | null; phone: string }) {
  ...
  const response = await fetch("https://api.mercadopago.com/checkout/preferences", { method: "POST", headers: { Authorization: `Bearer ${credentials.access_token}`, "Content-Type": "application/json" }, body: JSON.stringify({ items: [{ id: reservation.id, title: `${reservation.online_payment_kind === "full" ? "Pago completo" : "Apartado"} · Cabina de masajes`, quantity: 1, currency_id: "MXN", unit_price: reservation.online_payment_cents / 100 }], payer: { name: reservation.full_name, email: reservation.email || undefined, phone: { number: reservation.phone } }, external_reference: `cabin:${reservation.id}`, back_urls: { success: `${siteUrl}/cabina-masajes`, pending: `${siteUrl}/cabina-masajes`, failure: `${siteUrl}/cabina-masajes` }, notification_url: `${appUrl}/api/webhooks/mercadopago`, auto_return: "approved" }) });
