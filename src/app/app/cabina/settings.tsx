...
type Space = { id:string; active:boolean; slot_interval_minutes:number; booking_duration_minutes:number; capacity_per_slot:number; price_cents:number; deposit_enabled:boolean; deposit_percent:number; online_payment_options:("deposit" | "full")[] };
...
export function CabinSettings({ space, hours, reservations }: { space:Space|null; hours:Hour[]; reservations:Reservation[] }) {
  ...
  async function save() {
    ...
    const { error } = await db.rpc("save_rental_space_settings", { p_space_id:data.id, p_active:data.active, p_booking_duration_minutes:data.booking_duration_minutes, p_capacity_per_slot:data.capacity_per_slot, p_price_cents:spaceUpdate.price_cents, p_deposit_enabled:data.deposit_enabled, p_deposit_percent:data.deposit_percent, p_online_payment_options:data.online_payment_options, p_hours:schedule.map((hour) => ({ day_of_week:hour.day_of_week, opens_at:hour.opens_at, closes_at:hour.closes_at, active:hour.active })) });
    setBusy(false);
  ...
  return <>
    ...
    <section className="settings-card cabin-settings">
      ...
      <label className="switch-row"><span><strong>Pedir apartado en línea</strong><small>Si se activa, el cliente paga el porcentaje definido por Clip.</small></span><input type="checkbox" checked={data.deposit_enabled} onChange={(event) => setData({ ...data, deposit_enabled:event.target.checked })}/><i/></label>
      ...
      <fieldset className="pos-methods"><legend>Opciones de pago en la liga pública</legend><small>El cliente puede pagar el apartado, el total, o elegir entre ambos.</small><label><input type="checkbox" checked={data.online_payment_options.includes("deposit")} onChange={() => setData({ ...data, online_payment_options:data.online_payment_options.includes("deposit") ? data.online_payment_options.filter((item) => item !== "deposit") : [...data.online_payment_options, "deposit"] })}/> Apartado</label><label><input type="checkbox" checked={data.online_payment_options.includes("full")} onChange={() => setData({ ...data, online_payment_options:data.online_payment_options.includes("full") ? data.online_payment_options.filter((item) => item !== "full") : [...data.online_payment_options, "full"] })}/> Pago completo</label></fieldset>
