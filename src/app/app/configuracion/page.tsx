...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ seccion?: string }> }) {
  ...
  return <SettingsShell activeSection={activeSection} profileRole={profile.role} sections={visibleSections}>{activeSection === "agenda" && <WebAgendaSettings settings={settings} hours={businessHours ?? []}/>} {activeSection === "pagos" && <PaymentSettings settings={settings} integration={integration} />} {activeSection === "catalogo" && <CatalogManager categories={categories ?? []} services={services ?? []} products={products ?? []} />} {activeSection === "finanzas" && <FinanceSettings categories={financeCategories ?? []} tags={financeTags ?? []}/>} {activeSection === "cabina" && <CabinSettings space={cabinSpace} hours={cabinHours ?? []} reservations={cabinReservations ?? []}/>}</SettingsShell>;
}
