...
export default async function SettingsPage() {
  ...
  return <main className="ops-shell"><aside className="sidebar"><Link href="/" className="brand"><span>Ola</span> Bonita<small>BEAUTY SPA</small></Link><nav><Link href="/app">▦ <span>Agenda</span></Link>{profile.role === "owner" && <Link href="/app/equipo">♙ <span>Equipo</span></Link>}<Link href="/app/operacion">◇ <span>Ventas y caja</span></Link><Link href="/app/finanzas">◔ <span>Finanzas</span></Link>{["owner", "manager"].includes(profile.role) && <Link href="/app/cabina">▣ <span>Renta de cabina</span></Link>}<Link className="active" href="/app/configuracion">⚙ <span>Configuración</span></Link></nav><div className="sidebar-user"><span className="avatar">{profile.role.slice(0,2).toUpperCase()}</span><div><strong>Ola Bonita</strong><small>Configuración</small></div></div></aside><section className="ops-main settings-main"><PaymentSettings settings={settings} integration={integration} /><CatalogManager categories={categories ?? []} services={services ?? []} products={products ?? []} /></section></main>;
}
