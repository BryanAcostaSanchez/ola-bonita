...
export default async function AppDashboard() {
  ...
  const [{ data:bookings }, { data:weekBookings }, { data:cabinReservations }, { data:sales }, { data:cashSession }, { data:specialists }, { data:hours }, { data:weeklySpecialistHours }, { data:assignments }] = await Promise.all([
    ...
    supabase.from("specialist_hours").select("specialist_id, starts_at, ends_at").eq("day_of_week", dayOfWeek).eq("active", true),
    supabase.from("specialist_hours").select("specialist_id, day_of_week, starts_at, ends_at, active").eq("active", true),
  ...
  const specialistIdsWithServices = new Set(assignments?.map((assignment) => assignment.specialist_id) ?? []);
  const specialistIdsWithHours = new Set(hours?.map((hour) => hour.specialist_id) ?? []);
  const readySpecialists = specialists?.filter((specialist) => specialistIdsWithServices.has(specialist.id) && specialistIdsWithHours.has(specialist.id)) ?? [];
  ...
  return <main className="ops-shell">
    <aside className="sidebar"><Link href="/" className="brand"><span>Ola</span> Bonita<small>BEAUTY SPA</small></Link><nav><a className="active" href="#agenda">▦ <span>Agenda</span></a><Link href="/app/equipo">♙ <span>Equipo</span></Link><Link href="/app/operacion">◇ <span>Ventas y caja</span></Link><Link href="/app/analitica">◔ <span>Analítica</span></Link><Link href="/app/configuracion">⚙ <span>Configuración</span></Link></nav><div className="sidebar-user"><span className="avatar">{firstName.slice(0, 2).toUpperCase()}</span><div><strong>{profile.full_name}</strong><small>{profile.role === "owner" ? "Administración" : "Equipo Ola Bonita"}</small></div></div></aside>
