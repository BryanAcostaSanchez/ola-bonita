...
export default async function AppDashboard() {
  ...
  return <main className="ops-shell">
    <aside className="sidebar"><Link href="/" className="brand"><span>Ola</span> Bonita<small>BEAUTY SPA</small></Link><nav><a className="active" href="#agenda">▦ <span>Agenda</span></a><Link href="/app/operacion">◇ <span>Ventas y caja</span></Link><Link href="/app/analitica">◔ <span>Analítica</span></Link><Link href="/app/configuracion">⚙ <span>Configuración</span></Link></nav><div className="sidebar-user"><span className="avatar">{firstName.slice(0, 2).toUpperCase()}</span><div><strong>{profile.full_name}</strong><small>{profile.role === "owner" ? "Administración" : "Equipo Ola Bonita"}</small></div></div></aside>
