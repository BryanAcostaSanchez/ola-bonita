...
export default async function OperationPage() {
  ...
  const [{ data: profile }, { data: services }, { data: cashSession }] = await Promise.all([supabase.from("profiles").select("full_name, role").eq("id", user.id).single(), supabase.from("services").select("id,name,price_cents,category:service_categories(id,name)").eq("active", true).order("name"), supabase.from("cash_sessions").select("id,opening_float_cents,opened_at").eq("status", "open").order("opened_at", { ascending: false }).limit(1).maybeSingle()]);
