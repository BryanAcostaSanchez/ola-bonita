...
export async function POST(request: Request) {
  ...
  const booking = data?.[0]; if (!booking) return NextResponse.json({ error: "No pudimos crear la reserva." }, { status: 422 });
  ...
  const { data: emailBooking } = await admin.from("bookings").select("id, public_code, starts_at, price_cents, deposit_due_cents, customer:customers(full_name,email), service:services(name), specialist:profiles(full_name)").eq("id", booking.booking_id).single(); if (emailBooking) await sendBookingEmails(emailBooking as never).catch(() => undefined);
  try { const { data: paymentBooking, error: bookingError } = await admin.from("bookings").select("id, public_code, online_payment_cents, online_payment_kind, customer:customers(full_name, email, phone), service:services(name)").eq("id", booking.booking_id).single(); if (bookingError || !paymentBooking) throw new Error("Booking missing"); const checkout = await createCheckoutPreference(paymentBooking as never); if (!checkout?.checkoutUrl) throw new Error("Mercado Pago is not configured"); await Promise.all([admin.from("bookings").update({ payment_provider: "mercadopago", payment_preference_id: checkout.preferenceId }).eq("id", booking.booking_id), admin.from("payments").insert({ booking_id: booking.booking_id, amount_cents: amountCents, method: "online", provider: "mercadopago", provider_reference: checkout.preferenceId, status: "pending" })]); return NextResponse.json({ booking, checkout_url: checkout.checkoutUrl }, { status: 201, headers: { "Cache-Control": "no-store" } }); } catch { return NextResponse.json({ booking, error: "La reserva quedó registrada, pero no pudimos abrir el pago. Comunícate con Ola Bonita para completar el pago." }, { status: 201, headers: { "Cache-Control": "no-store" } }); }
}
