...
type Settings = { timezone: string; deposit_enabled: boolean; deposit_percent: number; lead_time_minutes: number; slot_interval_minutes: number } | null;
...
export function BookingServicePicker({ services, settings }: { services: Service[]; settings: Settings }) {
  ...
  async function submitBooking(event: FormEvent<HTMLFormElement>) {
    ...
    else { setBooking(data.booking ?? null); setStep(4); }
    setBusy(false);
  }
  ...
  if (step === 4 && booking && selected) return <section className="booking-form booking-success"><p className="eyebrow">RESERVA SOLICITADA</p><h2>Tu espacio está apartado.</h2><p>Tu código de reserva es <strong>{booking.public_code}</strong>. Recibirás los siguientes pasos por el medio de contacto que nos compartiste.</p><div className="booking-receipt"><span>{selected.name}</span><b>{money.format(booking.price_cents / 100)}</b>{booking.deposit_due_cents > 0 && <><span>Anticipo a pagar en línea ({booking.deposit_percent}%)</span><b>{money.format(booking.deposit_due_cents / 100)}</b><span>Saldo a liquidar en el spa</span><b>{money.format((booking.price_cents - booking.deposit_due_cents) / 100)}</b></>}</div><button className="button" onClick={() => window.location.assign("/")}>Volver al inicio <span>→</span></button></section>;
